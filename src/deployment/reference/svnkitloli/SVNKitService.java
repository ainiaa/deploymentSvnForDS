...
public class SVNKitService {
    ...
    public SVNKitService() {
        ...
        String localPathConf = rb.getString("svn.localpath");
        String svnUrlConf = rb.getString("svn.svnurl");
        String usernameConf = rb.getString("svn.username");
        String passwordConf = rb.getString("svn.password");
        this.localPath = localPathConf;
        DAVRepositoryFactory.setup();
        SVNRepositoryFactoryImpl.setup();
        FSRepositoryFactory.setup();
        SVNRepository repository = null;
        try {
            url = SVNURL.parseURIEncoded(svnUrlConf);
            repository = SVNRepositoryFactory.create(url, null);
        } catch (SVNException e) {
            e.printStackTrace();
        }
        ISVNAuthenticationManager authManager = new BasicAuthenticationManager(
                usernameConf, passwordConf);
        repository.setAuthenticationManager(authManager);
        util = new SVNKitUtil(url, repository, localPathConf, usernameConf, passwordConf);
    }
